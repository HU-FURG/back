...
export async function listReschedule(req: Request, res: Response) {
  ...
  const schema = z.object({
    ...
    type: z.enum(["user", "room"]).optional(),
  });

  const { page, bloco, number, id, type } = schema.parse(req.query);
  ...
  // 🔹 Filtro manual por número
  if (number) {
    filters.roomIdAmbiente = {
      contains: number,
      mode: "insensitive",
    };
  }

  // 🔥 Filtro dinâmico baseado no type
  if (id && type) {
    const parsedId = parseInt(id, 10);

    if (type === "user") {
      filters.OR = [{ createdById: parsedId }, { scheduledForId: parsedId }];
    }
    ...
    if (type === "room") {
      ...
      if (room) {
        filters.roomIdAmbiente = room.ID_Ambiente;
      }
    }
  }

  const total = await prisma.roomScheduleTemplate.count({
    where: filters,
  });
  ...
  const agendasComRoomId = await Promise.all(
    agendas.map(async (agenda) => {
      const room = await prisma.room.findFirst({
        where: {
          ID_Ambiente: agenda.roomIdAmbiente,
        },
        select: { id: true },
      });
...
export async function deleteReschedule(req: Request, res: Response) {
  ...
  if (hierarquia !== "boss" && template?.createdById !== userId) {
    return res.status(403).json({
      error: "Você não tem permissão para excluir este reagendamento.",
    });
  }
  ...
  await prisma.roomScheduleTemplate.delete({
    where: { id: id },
  });
